...
SELECT "QTGz8o0vcPm" data_element,
...
(SELECT ci.patient_id
	...
	WHERE ci.visit_date <= DATE(@enddate) AND ci.`chronic_illness` = 119481 AND ci.is_chronic_illness_controlled = 1065
	GROUP BY ci.`patient_id`)
AND p.`patient_id` IN
(SELECT ci.patient_id
	FROM kenyaemr_etl.etl_allergy_chronic_illness ci
	WHERE ci.visit_date <= DATE(@enddate) AND ci.`chronic_illness` = 117399 AND ci.is_chronic_illness_controlled = 1065
	GROUP BY ci.`patient_id`)
                                   ) tx GROUP BY tx.gender, tx.age_group
                                   
-- KP Section
UNION SELECT "NWx5oLEtJwf" data_element, (CASE t.key_population_type
...
-- PG Section
UNION SELECT "NWx5oLEtJwf" data_element, IF(t.pregnancy_status = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.pregnancy_status) `value` 
FROM (SELECT p.patient_id,
...
-- BF Section
UNION SELECT "NWx5oLEtJwf" data_element, IF(t.breastfeeding = 'Yes', 'BSjw9WXURmv', '')category_option, t.`organization_unit`, @period period, COUNT(t.breastfeeding) `value`
FROM (SELECT p.patient_id,
