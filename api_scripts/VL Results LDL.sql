...
SELECT "HNJfITs2a8Q" data_element,
...
(SELECT vl.patient_id FROM (SELECT fup.patient_id, de.patient_id AS started_on_drugs,
           MID(MAX(CONCAT(le.visit_date, IF(lab_test = 856, test_result, IF(lab_test=1305 AND test_result = 1302, "LDL",NULL)))),11) AS last_vl_result,
           IF(TIMESTAMPDIFF(MONTH,(MID(MAX(CONCAT(DATE(le.visit_date),DATE(date_test_requested))),11)),DATE(@endDate)) <= 12,'Valid','Invalid') AS validity,
           MAX(le.visit_date) AS last_vl_date
    FROM kenyaemr_etl.etl_patient_hiv_followup fup
    ...
    WHERE fup.visit_date <= DATE(@enddate)
    GROUP BY patient_id
    HAVING (started_on_drugs IS NOT NULL AND started_on_drugs <> '') AND validity = 'Valid' AND (last_vl_result = 'LDL' or last_vl_result < 50)) vl)                 
 ) tx GROUP BY tx.gender, tx.age_group

-- KP Section
UNION SELECT "iIEvEo5Vsms" data_element, (CASE t.key_population_type
...
-- PG Section
UNION SELECT "iIEvEo5Vsms" data_element, IF(t.pregnancy_status = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.pregnancy_status) `value` 
FROM (SELECT p.patient_id,
...
-- BF Section
UNION SELECT "iIEvEo5Vsms" data_element, IF(t.breastfeeding = 'Yes', 'BSjw9WXURmv', '')category_option, t.`organization_unit`, @period period, COUNT(t.breastfeeding) `value`
FROM (SELECT p.patient_id,
