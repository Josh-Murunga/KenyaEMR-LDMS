...
SELECT "j0WTcSkUjLm" data_element,
...
(SELECT t.patient_id
                FROM(
                    ...
                    GROUP BY patient_id
                    HAVING (started_on_drugs IS NOT NULL AND started_on_drugs <> '') AND establishment = 'Established' AND (
                        (
...
-- KP Section
UNION SELECT "eTZfOsKi1l1" data_element, (CASE t.key_population_type
...
-- PG Section
UNION SELECT "eTZfOsKi1l1" data_element, IF(t.pregnancy_status = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.pregnancy_status) `value` 
FROM (SELECT p.patient_id,
...
-- BF Section
UNION SELECT "eTZfOsKi1l1" data_element, IF(t.breastfeeding = 'Yes', 'BSjw9WXURmv', '')category_option, t.`organization_unit`, @period period, COUNT(t.breastfeeding) `value`
FROM (SELECT p.patient_id,
