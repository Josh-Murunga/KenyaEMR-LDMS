...
SELECT "z5gaAKe0Wo4" data_element,
...
SELECT  
e.gender, 
e.`DOB`, TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) AS age, ou.organization_unit,
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) < 1, '< 1yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 1 AND 4, '1-4yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 5 AND 9, '5-9yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 10 AND 14, '10-14yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 15 AND 19, '15-19yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 20 AND 24, '20-24yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 25 AND 29, '25-29yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 30 AND 34, '30-34yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 35 AND 39, '35-39yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 40 AND 44, '40-44yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 45 AND 49, '45-49yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 50 AND 54, '50-54yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 55 AND 59, '55-59yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) BETWEEN 60 AND 64, '60-64yrs',
	IF(TIMESTAMPDIFF(YEAR,e.`DOB`,@enddate) > 64, '65+yrs', NULL))))))))))))))) age_group,
...
-- KP section
UNION SELECT "ez6EDFfNXFO" data_element, (CASE t.key_population_type
...
GROUP BY e.patient_id  
HAVING TI_on_art=0 
)t WHERE t.key_population_type IS NOT NULL GROUP BY t.key_population_type

-- PG section
UNION SELECT "ez6EDFfNXFO" data_element, IF(t.pregnancy_status = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.pregnancy_status) `value`    
FROM (  
SELECT e.patient_id,e.date_started,  
e.gender, 
e.dob, 
...
GROUP BY e.patient_id  
HAVING TI_on_art=0 
)t WHERE t.pregnancy_status = 'Yes' GROUP BY t.pregnancy_status

-- BF section
UNION SELECT "ez6EDFfNXFO" data_element, IF(t.breastfeeding = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.breastfeeding) `value`    
FROM (  
SELECT e.patient_id,e.date_started,  
e.gender, 
e.dob, 
...
GROUP BY e.patient_id  
HAVING TI_on_art=0 
)t WHERE t.breastfeeding = 'Yes' GROUP BY t.breastfeeding;