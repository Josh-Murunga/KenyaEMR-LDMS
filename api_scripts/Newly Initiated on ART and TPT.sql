...
SELECT "Bi1noiNPHyH" data_element,
...
AND p.`patient_id` IN
(SELECT init.patient_id 
FROM kenyaemr_etl.etl_ipt_initiation init 
INNER JOIN kenyaemr_etl.etl_patient_demographics d ON init.patient_id = d.patient_id 
AND init.voided = 0 AND d.voided = 0
WHERE DATE(init.visit_date) BETWEEN DATE(@startDate) AND DATE(@endDate))
                                   ) tx GROUP BY tx.gender, tx.age_group

-- KP Section
UNION SELECT "hpTDvQoe39w" data_element, (CASE t.key_population_type
...
-- PG Section
UNION SELECT "hpTDvQoe39w" data_element, IF(t.pregnancy_status = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.pregnancy_status) `value` 
FROM (SELECT p.patient_id,
...
-- BF Section
UNION SELECT "hpTDvQoe39w" data_element, IF(t.breastfeeding = 'Yes', 'BSjw9WXURmv', '')category_option, t.`organization_unit`, @period period, COUNT(t.breastfeeding) `value`
FROM (SELECT p.patient_id,
