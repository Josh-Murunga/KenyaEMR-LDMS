...
SELECT "peBT831F73b" data_element,
...
-- KP Section
UNION SELECT "jxMgmOqxhRj" data_element, (CASE t.key_population_type
...
-- PG Section
UNION SELECT "jxMgmOqxhRj" data_element, IF(t.pregnancy_status = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.pregnancy_status) `value` 
FROM (SELECT p.patient_id,
...
-- BF Section
UNION SELECT "jxMgmOqxhRj" data_element, IF(t.breastfeeding = 'Yes', 'BSjw9WXURmv', '')category_option, t.`organization_unit`, @period period, COUNT(t.breastfeeding) `value`
FROM (SELECT p.patient_id,
