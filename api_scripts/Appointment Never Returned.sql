...
SELECT "Iay4I5c7Hon" data_element,
...
DATE(d.visit_date) >= DATE(@endDate)) AND Appointment_status = 'Never Returned' ) a)     
 ) tx GROUP BY tx.gender, tx.age_group

-- KP Section
UNION SELECT "GBPZcc7iGcW" data_element, (CASE t.key_population_type
...
-- PG Section
UNION SELECT "GBPZcc7iGcW" data_element, IF(t.pregnancy_status = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.pregnancy_status) `value` 
FROM (SELECT p.patient_id,
...
-- BF Section
UNION SELECT "GBPZcc7iGcW" data_element, IF(t.breastfeeding = 'Yes', 'BSjw9WXURmv', '')category_option, t.`organization_unit`, @period period, COUNT(t.breastfeeding) `value`
FROM (SELECT p.patient_id,
