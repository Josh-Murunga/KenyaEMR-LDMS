...
SELECT "Wyz94SCPhsO" data_element,
...
(SELECT vl.patient_id FROM (SELECT fup.patient_id, de.patient_id AS started_on_drugs,
    ...
    WHERE fup.visit_date <= DATE(@enddate)
    GROUP BY patient_id
    HAVING (started_on_drugs IS NOT NULL AND started_on_drugs <> '') AND validity = 'Valid') vl)                 
 ) tx GROUP BY tx.gender, tx.age_group

-- KP Section
UNION SELECT "N2Ca2bCaGKk" data_element, (CASE t.key_population_type
...
-- PG Section
UNION SELECT "N2Ca2bCaGKk" data_element, IF(t.pregnancy_status = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.pregnancy_status) `value` 
FROM (SELECT p.patient_id,
...
-- BF Section
UNION SELECT "N2Ca2bCaGKk" data_element, IF(t.breastfeeding = 'Yes', 'BSjw9WXURmv', '')category_option, t.`organization_unit`, @period period, COUNT(t.breastfeeding) `value`
FROM (SELECT p.patient_id,
