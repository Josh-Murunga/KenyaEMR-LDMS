...
SELECT "z0E0yIDkvto" data_element,
...
WHERE f.visit_date <=DATE(@endDate) AND f.next_appointment_date IS NOT NULL GROUP BY f.patient_id HAVING months_tca >= 165) f)     
     GROUP BY t.gender, t.age_group) tx
	 
-- KP section	 
UNION SELECT "QmRBmTrIARp" data_element, (CASE t.key_population_type
		...
		COUNT(t.key_population_type) `value` 
                FROM(
                    ...
                    (SELECT f.patient_id FROM (SELECT f.patient_id,TIMESTAMPDIFF(DAY,MAX(f.visit_date),MID(MAX(CONCAT(f.visit_date,f.next_appointment_date)),11))  months_tca FROM kenyaemr_etl.etl_patient_hiv_followup f
WHERE f.visit_date <=DATE(@endDate) AND f.next_appointment_date IS NOT NULL GROUP BY f.patient_id HAVING months_tca >= 165) f) AND t.key_population_type IS NOT NULL GROUP BY t.key_population_type
				
-- PG section				
UNION SELECT "QmRBmTrIARp" data_element, IF(t.pregnancy_status = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.pregnancy_status) `value`
                FROM(
                    SELECT fup.visit_date,fup.patient_id, MAX(e.visit_date) AS enroll_date, ou.`organization_unit`,
                           GREATEST(MAX(e.visit_date), IFNULL(MAX(DATE(e.transfer_in_date)),'0000-00-00')) AS latest_enrolment_date,
                           GREATEST(MAX(fup.visit_date), IFNULL(MAX(d.visit_date),'0000-00-00')) AS latest_vis_date,
                    ...
                    (SELECT f.patient_id FROM (SELECT f.patient_id,TIMESTAMPDIFF(DAY,MAX(f.visit_date),MID(MAX(CONCAT(f.visit_date,f.next_appointment_date)),11))  months_tca FROM kenyaemr_etl.etl_patient_hiv_followup f
WHERE f.visit_date <=DATE(@endDate) AND f.next_appointment_date IS NOT NULL GROUP BY f.patient_id HAVING months_tca >= 165) f) AND t.pregnancy_status = 'Yes' GROUP BY t.pregnancy_status
                
-- BF section				
UNION SELECT "QmRBmTrIARp" data_element, IF(t.breastfeeding = 'Yes', 'BSjw9WXURmv', '')category_option, t.`organization_unit`, @period period, COUNT(t.breastfeeding) `value`
                FROM(
                    SELECT fup.visit_date,fup.patient_id, MAX(e.visit_date) AS enroll_date, ou.`organization_unit`,
                           GREATEST(MAX(e.visit_date), IFNULL(MAX(DATE(e.transfer_in_date)),'0000-00-00')) AS latest_enrolment_date,
                           GREATEST(MAX(fup.visit_date), IFNULL(MAX(d.visit_date),'0000-00-00')) AS latest_vis_date,
                    ...
                    (SELECT f.patient_id FROM (SELECT f.patient_id,TIMESTAMPDIFF(DAY,MAX(f.visit_date),MID(MAX(CONCAT(f.visit_date,f.next_appointment_date)),11))  months_tca FROM kenyaemr_etl.etl_patient_hiv_followup f
WHERE f.visit_date <=DATE(@endDate) AND f.next_appointment_date IS NOT NULL GROUP BY f.patient_id HAVING months_tca >= 165) f) AND t.breastfeeding = 'Yes' GROUP BY t.breastfeeding;
