...
SELECT "V32wXNSoKs5" data_element,
...
FROM (SELECT t.patient_id, "GniE4hPeaIJ" data_element, (CASE t.key_population_type
...
-- PG section	
UNION SELECT rtt.data_element, rtt.category_option, rtt.organization_unit, rtt.period, rtt.value
FROM (SELECT t.patient_id, "GniE4hPeaIJ" data_element, IF(t.pregnancy_status = 'Yes', 'um52UVweKbK', '')category_option, t.`organization_unit`, @period period, COUNT(t.pregnancy_status) `value`
                FROM(
                    SELECT fup.visit_date,fup.patient_id, MAX(e.visit_date) AS enroll_date, ou.`organization_unit`,
                           GREATEST(MAX(e.visit_date), IFNULL(MAX(DATE(e.transfer_in_date)),'0000-00-00')) AS latest_enrolment_date,
                           GREATEST(MAX(fup.visit_date), IFNULL(MAX(d.visit_date),'0000-00-00')) AS latest_vis_date,
...
-- BF section	
UNION SELECT rtt.data_element, rtt.category_option, rtt.organization_unit, rtt.period, rtt.value
FROM (SELECT t.patient_id, "GniE4hPeaIJ" data_element, IF(t.breastfeeding = 'Yes', 'BSjw9WXURmv', '')category_option, t.`organization_unit`, @period period, COUNT(t.breastfeeding) `value`
                FROM(
                    SELECT fup.visit_date,fup.patient_id, MAX(e.visit_date) AS enroll_date, ou.`organization_unit`,
                           GREATEST(MAX(e.visit_date), IFNULL(MAX(DATE(e.transfer_in_date)),'0000-00-00')) AS latest_enrolment_date,
                           GREATEST(MAX(fup.visit_date), IFNULL(MAX(d.visit_date),'0000-00-00')) AS latest_vis_date,
